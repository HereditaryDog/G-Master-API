...
- 基于上游 `new-api v1.0.0-rc.5` 完成第一轮选择性同步，保留现有 G-Master 生产前端与品牌页面，不引入上游新前端重写。
...
- 将分支从 `v0.13.1-gmaster.3` 同步至上游 `new-api v0.13.2`，合入模型同步、分层计费、上游价格同步、配置解析、Responses 工具调用和 Ali 渠道原生 Messages 相关修复。
- 更新源码构建、Docker 构建、前端展示版本和 Apifox 导出版本号，统一为 `v0.13.2-gmaster.1`。
- 保留当前 G-Master 控制台、模型广场、品牌首页与生产部署视觉体系，暂不追随上游 `v1.0.0-alpha.1` / `main` 的新前端重写。
...
- 上游价格同步支持从 pricing endpoint 同步分层计费、倍率、补全倍率、缓存、图片和音频价格字段。
...
- 修复上游模型检查时未加载模型映射的问题，并在模型拉取弹窗中显示已从上游移除的模型。
...
- 将分支从 `v0.12.15-gmaster.1` 同步至上游 `new-api v0.13.1`，补齐 DeepSeek V4 推理后缀处理、分层计费模型列表和计费配置检测修复。
- 合入上游分层计费表达式能力、动态定价展示、用量日志与倍率设置相关更新，同时保留 `G-Master API` 品牌、生产部署配置、文档入口和现有充值/分组升级逻辑。
...
- 移除上游 nightly DockerHub 发布工作流，避免 G-Master 分支误推送到上游 `calciumion/new-api` 镜像命名空间。
...
- 将分支从上游 `new-api v0.12.11` 同步至 `v0.12.14`，同时保留 `G-Master API` 品牌、现有部署地址与分组充值升级逻辑。
- 合入上游对 `Claude Opus 4.7` 的支持，并同步更新 Claude / Vertex / AWS 适配常量与相关倍率设置。
