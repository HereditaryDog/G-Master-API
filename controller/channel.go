...
import (
	...
	relaychannel "github.com/yangjunyu/G-Master-API/relay/channel"
...
func GetAllChannels(c *gin.Context) {
	...
	if enableTagMode {
		...
		baseQuery := buildChannelListQuery(groupFilter, statusFilter, typeFilter)
...
func SearchChannels(c *gin.Context) {
	...
	if enableTagMode {
		...
		for _, tag := range tags {
			if tag != nil && *tag != "" {
				var tagChannels []*model.Channel
				tagQuery := buildChannelListQuery(group, -1, -1).Where("tag = ?", *tag)
				if modelKeyword != "" {
					tagQuery = tagQuery.Where("models LIKE ?", "%"+modelKeyword+"%")
				}
				err := sortOptions.Apply(tagQuery).Omit("key").Find(&tagChannels).Error
				if err != nil {
					c.JSON(http.StatusOK, gin.H{
						"success": false,
						"message": err.Error(),
