...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		result, completeErr := model.CompleteTopUpByTradeNo(verifyInfo.ServiceTradeNo, model.PaymentProviderEpay, verifyInfo.Type)
		if completeErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 回调处理订单失败 trade_no=%s callback_type=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), completeErr.Error()))
			return
		}
		if result == nil {
			return
		...
		if result.AlreadySuccess {
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 回调订单已处理 trade_no=%s user_id=%d client_ip=%s", verifyInfo.ServiceTradeNo, result.UserId, c.ClientIP()))
			return
		}
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f", verifyInfo.ServiceTradeNo, result.UserId, c.ClientIP(), result.QuotaToAdd, result.PaymentMoney))
		model.RecordTopupLog(result.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(result.QuotaToAdd), result.PaymentMoney), c.ClientIP(), result.PaymentMethod, "epay")
	} else {
